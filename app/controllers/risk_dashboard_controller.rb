class RiskDashboardController < ApplicationController
  ...
  def index
    ...
    @risks_by_confidentiality = Risk.joins(:project).where(projects: { status: Project::STATUS_ACTIVE })
                                     .group(:confidentiality).count.transform_keys { |k| k || 0 }
    @risks_by_integrity = Risk.joins(:project).where(projects: { status: Project::STATUS_ACTIVE })
                               .group(:integrity).count.transform_keys { |k| k || 0 }
    @risks_by_availability = Risk.joins(:project).where(projects: { status: Project::STATUS_ACTIVE })
                                  .group(:availability).count.transform_keys { |k| k || 0 }

    # Risk significance data
    @avg_significance = Risk.joins(:project)
                             .where(projects: { status: Project::STATUS_ACTIVE })
                             .where.not(level_of_significance: nil)
                             .average(:level_of_significance)
    @max_significance = Risk.joins(:project)
                             .where(projects: { status: Project::STATUS_ACTIVE })
                             .maximum(:level_of_significance)

    # Top risks by significance
    @top_risks_by_significance = Risk.joins(:project)
                                      .where(projects: { status: Project::STATUS_ACTIVE })
                                      .where(closed_on: nil)
                                      .where.not(level_of_significance: nil)
  ...
  def project
    ...
    # CIA distribution data for charts
    @risks_by_confidentiality = project_risks.group(:confidentiality).count.transform_keys { |k| k || 0 }
    @risks_by_integrity = project_risks.group(:integrity).count.transform_keys { |k| k || 0 }
    @risks_by_availability = project_risks.group(:availability).count.transform_keys { |k| k || 0 }
